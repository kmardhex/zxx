...
echo -e "${YELLOW} Halo,${WHITE} $(whoami) ${YELLOW}- Welcome To Cyber Indonesia ${RESET}"
echo -e "${CYAN} Tanggal:${WHITE} $(date +'%A, %d %B %Y') ${CYAN}| Waktu:${WHITE} $(date +'%T')${RESET}"
echo -e "${CYAN} Kernel:${WHITE} $(uname -srmo)${RESET}"
echo -e "${CYAN} Uptime:${WHITE} $(uptime -p 2>/dev/null || echo \"n/a\")${RESET}"

echo -e "${PINK}────────────────────────────────────────────────────${RESET}"
echo -e "${BOLD}${WHITE} Tips:${RESET} ketik ${YELLOW}pkg up && pkg install neofetch${RESET} untuk info sistem lebih rapi."
echo